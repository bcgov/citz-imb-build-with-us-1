...
const { currentMigration, lastAddedMigration } = require('./migrations.json');
...
exports.checkForNewMigrations = async () => {
  console.log(`${colors.Yellow}DB Migrations: ${colors.Reset}Checking for new migrations...`);
...
const runMigrations = async () => {
  ...
  console.log(`${colors.Yellow}DB Migrations: ${colors.Reset}Found ${numMigrations} migration(s), ${currentMigration + 1} to ${lastAddedMigration}...`);

  // Do each migration, one at a time.
  for (let migration = 1; migration <= numMigrations; migration++) {
    console.log(`${colors.Yellow}DB Migrations: ${colors.Reset}Running migration ${migration} of ${numMigrations}...`);
    await readMigrationFiles(currentMigration + migration);
    // Update the migrations.json.
    await updateJsonProperty('currentMigration', currentMigration + migration);
  }
};
...
const readMigrationFiles = async (migration) => {
  ...
  fs.readdir(baseDirectory, (error, files) => {
    if (error) {
      console.error(`${colors.Yellow}DB Migrations: ${colors.Pink}Error reading migrations directory${colors.Reset}`, error);
      return;
    }
      ...
      if (fileRemovedLeadingZeros.startsWith(migration.toString())) {
        ...
        return new Promise((resolve, reject) => {
          fs.stat(filePath, (error, stat) => {
            if (error) {
              console.error(`${colors.Yellow}DB Migrations: ${colors.Pink}Error with path to migration directory${colors.Reset}`, error);
              reject(error);
            }
            ...
            if (stat.isDirectory()) {
              ...
              fs.readdir(filePath, (error, files) => {
                if (error) {
                  console.error(`${colors.Yellow}DB Migrations: ${colors.Pink}Error reading migration directory at path ${colors.Reset}${filePath}`, error);
                  reject(error);
                }
                ...
                files.forEach(file => {
                  if (file.endsWith('.sql')) {
                    // Read SQL File.
                    fs.readFile(path.join(filePath, file), 'utf8', (error, fileData) => {
                      console.log(`${colors.Yellow}DB Migrations: ${colors.Reset}Reading file`, file);
                      if (error) {
                        console.error(`${colors.Yellow}DB Migrations: ${colors.Pink}Error reading file at ${colors.Reset}${filePath}`, error);
                        reject(error);
                      }
                      data.push(fileData);
    ...
    .then(async (sqls) => {
      ...
      if (Array.isArray(sqls)) sqls.forEach(async (sql) => querySQLScript(sql));
...
const updateJsonProperty = async (propertyName, newValue) => {
  try {
      ...
      await writeFile('./database/migrations.json', JSON.stringify(jsonData, null, 2), 'utf8');
  } catch (error) {
      console.error(`${colors.Yellow}DB Migrations: ${colors.Pink}Error updating JSON File${colors.Reset}`, error);
  }
};
